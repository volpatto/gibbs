...
from gibbs.cpp_wrapper import _cubic_polynomial_roots, _cubic_polynomial_real_positive_roots
...
def test_polynomial_roots(coeffs):
    ...
    real_roots_eos = _cubic_polynomial_roots(coeffs[::-1])  # the order is reversed compared with numpy
...
def test_known_real_positive_roots(coeffs, real_roots):
    real_roots_eos = _cubic_polynomial_real_positive_roots(coeffs[::-1])
